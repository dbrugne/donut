...
module.exports = {
  ...
  mentionTemplate: _.template('<strong><% if (mention.type === \'room\') { %><a href="'+conf.url+'/room/<%= mention.title %>" style="<%= options.style %>"><%= mention.title %></a><% } else if (mention.type === \'user\') { %><a href="'+conf.url+'/user/<%= mention.title.replace(\'@\', \'\') %>" style="<%= options.style %>"><%= mention.title %></a><% } %></strong>'),
